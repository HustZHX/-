...
def load_data_set(file_name):
    ...
    with open(file_name) as file_obj:
        ...
        # 随机划分数据集为训练集 和 测试集 7:3 即2218:950
...
def test_bayes():
    file_name = 'C:\\Users\\10094\\PycharmProjects\\项目\\voice.csv'
    ...
    for i in list(range(len(test_mat))):
        ...
        if test_classes[i] == 1: man_count+=1
        if result == test_classes[i]:
            correct_count += 1
            if result==1:
                print((count),':预测正确!是男性')
                man_correct += 1
            if result == 0: print((count),':预测正确!是女性')
        if result != test_classes[i]: print('失败')

        count += 1

    correct_rate = 0.0
    man_correct_rate = 0.0
    woman_correct_rate = 0.0
    correct_rate=correct_count / count
    man_correct_rate=man_correct/man_count
    woman_correct_rate=(correct_count-man_correct)/(count-man_count)

    print(100*correct_rate,'%')
    print('man:',man_correct_rate)
    print('woman:', woman_correct_rate)
